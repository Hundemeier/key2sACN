    ...
    <tbody>
      <tr v-for="device in Devices" :key="device.id" :class="{'table-success': device.listening}">
        ...
        <td v-if="device.listening">
          <button class="btn btn-outline-secondary btn-sm full-width" style="width: 100%" @click="setListen(device.id, false)">Stop</button>
        </td>
        <td v-else>
          <button class="btn btn-outline-secondary btn-sm full-width" style="width: 100%" @click="setListen(device.id, true)">Listen</button>
        </td>
      </tr>
    </tbody>
  </table>
